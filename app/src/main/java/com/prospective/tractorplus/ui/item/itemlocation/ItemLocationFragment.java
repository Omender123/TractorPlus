package com.prospective.tractorplus.ui.item.itemlocation;
...
import com.prospective.tractorplus.databinding.FragmentItemLocationBinding;
   ...
   // public ItemClickCallback callback;
        ...
        try {
            if (getActivity() != null) {
                if (getActivity().getIntent().getExtras() != null) {
                    ...
                    itemViewModel.townshipId = getActivity().getIntent().getStringExtra(Constants.ITEM_TOWNSHIP_TYPE_ID);
    ...
    protected void initAdapters() {
        ...
        ItemLocationAdapter nvadapter = new ItemLocationAdapter(dataBindingComponent,
                new ItemLocationAdapter.NewsClickCallback() {
                    @Override
                    public void onClick(ItemLocation itemLocation, String id) {

                        if (ItemLocationFragment.this.getActivity() != null) {

                            if (ItemLocationFragment.this.getActivity().getIntent().getStringExtra(Constants.LOCATION_FLAG).equals(Constants.LOCATION_NOT_CLEAR_ICON)) {

                                if (Config.ENABLE_SUB_LOCATION) {

                                    if(itemLocation.name.equals(Constants.LOCATION_ALL))
                                    {
                                        navigationController.navigateToMainActivity(ItemLocationFragment.this.getActivity(), "", Constants.LOCATION_ALL, "", Constants.LOCATION_ALL, itemViewModel.itemId, Constants.INVALID_LAT_LNG, Constants.INVALID_LAT_LNG);
                                        ItemLocationFragment.this.getActivity().finish();

                                    } else {

                                        navigationController.navigateToTownshipLocationActivity(ItemLocationFragment.this.getActivity(), Constants.SELECT_LOCATION_FROM_CITY, itemLocation.id, itemLocation.name, itemViewModel.itemId, itemLocation.lat, itemLocation.lng);
                                    }

                                } else {

                                    navigationController.navigateToMainActivity(ItemLocationFragment.this.getActivity(), itemLocation.id, itemLocation.name, selected_township_id, selected_township_name, itemViewModel.itemId, itemLocation.lat, itemLocation.lng);
                                    ItemLocationFragment.this.getActivity().finish();
                                }


                            } else if (ItemLocationFragment.this.getActivity().getIntent().getStringExtra(Constants.LOCATION_FLAG).equals(Constants.SELECT_LOCATION_FROM_HOME)) {

                                if (Config.ENABLE_SUB_LOCATION) {

                                    if(itemLocation.name.equals(Constants.LOCATION_ALL))
                                    {
                                        navigationController.navigateBackToMainActivity(ItemLocationFragment.this.getActivity(), "", Constants.LOCATION_ALL, "", Constants.LOCATION_ALL, Constants.INVALID_LAT_LNG, Constants.INVALID_LAT_LNG);
                                        ItemLocationFragment.this.getActivity().finish();

                                    } else {
                                        navigationController.navigateToTownshipLocationActivity(ItemLocationFragment.this.getActivity(), Constants.SELECT_LOCATION_FROM_HOME, itemLocation.id, itemLocation.name, itemViewModel.itemId, itemLocation.lat, itemLocation.lng);
                                    }

                                } else {
                                    navigationController.navigateBackToMainActivity(ItemLocationFragment.this.getActivity(), itemLocation.id, itemLocation.name, itemLocationViewModel.townshipId, itemLocationViewModel.townshipName, itemLocation.lat, itemLocation.lng);
                                    ItemLocationFragment.this.getActivity().finish();
                                }
                            } else {
        ...
        private void assignLocationId(String locationId, String townshipId) {
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (data != null) {
            ...
            }else if(requestCode == Constants.REQUEST_CODE__SELECTED_CITY_FRAGMENT) {
                if(getActivity() != null ) {
                    getActivity().setResult(Constants.RESULT_CODE__SEARCH_WITH_ITEM_LOCATION_TYPE, data);
                    getActivity().finish();
                }
            }
        }
    ...
    private void replaceData(List<ItemLocation> categoryList) {
        ...
        for (int i = 0; i < categoryList.size(); i++) {
            ...
            ItemLocation itemLocation = new ItemLocation(Constants.ZERO, Constants.LOCATION_ALL, "", "", "", "", "", "", "", "", "");
